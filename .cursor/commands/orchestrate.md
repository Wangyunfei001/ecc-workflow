...
必须产出结构化的交接对象，存储在 `docs/handoffs/handoff-<目标Agent>-<时间戳>.yml`，或在调用下游 Agent 时作为附带参数。

```yaml
# handoff.yml（由上游生成，下游读取）
from: planner
to: spec-writer  
spec_inputs:
  requirement: docs/requirements/2026-03-23-login.md
  plan: docs/plans/2026-03-23-login.md
key_decisions:
  - "使用 JWT，不用 session"
  - "手机号使用 E.164 格式"
blockers: []
