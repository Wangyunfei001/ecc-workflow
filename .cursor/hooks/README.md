...
本插件现默认推荐使用基于 Node.js 的 `skills/continuous-learning/hooks/observe.mjs` 处理 Hook 监控，以获得更好的 JSON 处理和错误恢复。原有 `observe.sh` 将作为兼容向后转发给 Node 脚本的薄封装。

`observe.mjs` (及兼容层 `observe.sh`) 采用以下优先级读取上下文：
