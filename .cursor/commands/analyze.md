...
当需求分析完成并输出需求文档后，**必须**在回复末尾显示以下引导块：
...
📄 需求文档已保存: `docs/requirements/YYYY-MM-DD-<name>.md`
📊 追问清单完成度: [X]%
...
👉 **下一步操作**:
1. 审查需求文档，将 `status: clarified` → `status: approved`
2. 然后执行: `@planner @docs/requirements/<name>.md` 进入任务规划
```

> **⚠️ 规则**: 此引导块为强制输出项，不可省略。即使在 `/orchestrate` 编排流程中也必须显示。
